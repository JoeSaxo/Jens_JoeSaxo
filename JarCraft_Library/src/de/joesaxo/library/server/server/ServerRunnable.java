...
class ServerRunnable extends Thread {
	...
	private int getClientId(String client) {
		for (int i = 0; i < connectedClients(); i++) {
			if (clients[i].getClient().equals(client)) {
				return i;
			}
		}
		return -1;
	...
	boolean disconnect(String clientid) {
		...
		clients[getClientId(clientid)].stopClient();
		return true;
	}
	...
	boolean isConnected(String client) {
		for (DevClient sClient : clients) {
			if (sClient.getClient() == client) return sClient.isConnected();
		}
		return false;
	}
	...
	String getClient(int clientid) {
		...
		return clients[clientid].getClient();
	}
	...
	boolean Send(String client, String message) {
		return clients[getClientId(client)].send(message);
	}
