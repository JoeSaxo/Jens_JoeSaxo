 ...
 */
public class AdvancedPluginLoader extends PluginLoader {
    ...
    public static boolean annotationMatchesParameter(Annotation annotation, String name, Object value) {
        for (Method method : getMethodsFromAnnotation(annotation)) {
                if (method.getName().equals(name)) {
                    if (invokeMethod(method, annotation).equals(value)) return true;
                }
        }
        return false;
    }
    ...
    public static <C> List<Class<C>> classFilter(List<Class<C>> classes, AnnotationModule annotationParrameter) {
        List<Class<C>> annotatedClasses = new ArrayList<>();
        for (Class<C> cls : classes) {
            if (getAnnotationFromModule(cls, annotationParrameter) != null) {
                annotatedClasses.add(PluginLoader.convert(cls));
            }
        }
